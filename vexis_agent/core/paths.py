...
def vexis_dir() -> Path:
    """Operational state (config, curator state, logs). Default `~/.vexis/`.
    ...
    Override with the ``VEXIS_HOME`` environment variable (e.g. for
    test isolation, container deploys, or running multiple instances
    on one box). The env var is re-read on every call so the value
    can change between invocations — matches the CLAUDE.md "config
    reads disk per call" invariant.
    """
    raw = os.environ.get("VEXIS_HOME")
    path = Path(raw).expanduser() if raw else Path.home() / ".vexis"
    path.mkdir(parents=True, exist_ok=True)
    return path
