...
from vexis_agent.core.memory import ENTRY_DELIMITER as _MEMORY_ENTRY_DELIMITER
from vexis_agent.core.paths import memories_dir, skills_dir, user_candidates_path
from vexis_agent.core.skills import PinStore, discover_skills
...
# Day 3 (v2 §3.4): USER.md-specific patterns are now defined in
# ``core/identity_threat.py`` so they can be shared with
# ``MemoryStore._scan_for_threats`` (target=user) without a circular
# import. Re-exported here for backward compatibility with tests and
# external callers.
from vexis_agent.core.identity_threat import (  # noqa: E402
    _NON_PERSON_CAPITALIZED,
    _THIRD_PARTY_PATTERNS,
    _USER_MD_THREAT_PATTERNS,
    check_named_third_party as _check_named_third_party,
)


# --------------------------------------------------------------------
# Named-third-party check (Day 3.5; moved to core/identity_threat.py)
# --------------------------------------------------------------------
#
...
# system prompt. Implementation lives in ``core/identity_threat.py``
# alongside the other USER.md patterns so MemoryStore._scan_for_threats
# can fire it on writes that don't go through the curator's
# _validate_lesson. ``_check_named_third_party`` is re-exported
# above as the legacy name for tests and external callers.
